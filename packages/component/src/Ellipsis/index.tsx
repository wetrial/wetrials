...
export default class Ellipsis extends Component<IEllipsisProps, any> {
  ...
  computeLine = () => {
    const { lines } = this.props;
    if (lines && !isSupportLineClamp && this.root.current != null) {
      const text = this.shadowChildren.innerText || this.shadowChildren.textContent;

      const lineHeight = parseInt(getComputedStyle(this.root.current).lineHeight || '', 10);
  ...
  render() {
    ...
    const id = `wetrial-ellipsis-${`${new Date().getTime()}${Math.floor(Math.random() * 100)}`}`;
    ...
    return (
      <div {...restProps} ref={this.root} className={cls}>
